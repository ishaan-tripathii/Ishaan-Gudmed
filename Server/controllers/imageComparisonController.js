...
export const getImageComparison = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error fetching image comparison data", error });
  }
};
...
export const updateImageComparison = async (req, res) => {
  try {
    ...
    if (!data) {
      ...
      await newData.save();
      notifyClients("imageComparisonUpdated");
    ...
    res.status(500).json({ message: "Error updating image comparison data", error });
  }
};