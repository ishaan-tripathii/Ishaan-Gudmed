import gudmedTransforms from "../models/gudmedtodayModel.js";
import { notifyClients } from "../services/socket.js";

// Fetch Gudmed Today data
export const getGudmedToday = async (req, res) => {
    try {
        const data = await gudmedTransforms.find();
...
export const createGudmedToday = async (req, res) => {
    try {
        ...
        const newGudmedToday = new gudmedTransforms({
            heading,
            description,
            cards,
            future
...
export const updateGudmedToday = async (req, res) => {
    try {
        ...
        const exists = await gudmedTransforms.findById(req.params.id);
        if (!exists) {
        ...
        const updatedGudmedToday = await gudmedTransforms.findByIdAndUpdate(
            req.params.id,
...
export const deleteGudmedToday = async (req, res) => {
    try {
        const deletedData = await gudmedTransforms.findByIdAndDelete(req.params.id);
