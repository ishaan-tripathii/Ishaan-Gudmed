...
const PageForm = ({ pageToEdit, onClose, isOpen }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const url = pageToEdit ? `http://localhost:5000/api/pages/${pageToEdit._id}` : "http://localhost:5000/api/pages"
    ...
    <AnimatePresence>
      ...
      >
        <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
          <div className="p-6">
            <h3 className="text-2xl font-bold text-gray-800 mb-4">{pageToEdit ? "Edit Slide" : "Create New Slide"}</h3>
